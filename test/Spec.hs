  ...
  describe "Testing some SemVer examples" $ do
    ...
    it "1.0.0-alpha+001" $ do
      case parse "1.0.0-alpha+001" of
        Failure err -> fail (show err)
        Success res ->
          res `shouldBe` SemVer 1 0 0 [NOSS "alpha"] [NOSS "001"]

    it "1.0.0+20130313144700" $ do
      case parse "1.0.0-beta" of
        Failure err -> fail (show err)
        Success res ->
          res `shouldBe` SemVer 1 0 0 [NOSS "beta"] [NOSS "20130313144700"]

    it "1.0.0-beta+exp.sha.5114f85" $ do
      case parse "1.0.0-beta" of
        Failure err -> fail (show err)
        Success res ->
          res `shouldBe` SemVer 1 0 0 [NOSS "beta"] [NOSS "exp", NOSS "sha", NOSS "5114f85"]
